...
const JETPACK_MODEL_PATH := "res://models/jetpack/model.glb"

var visual_root: Node3D
var pickup_kind := "jetpack"
...
func _process(delta: float) -> void:
	...
	visual_root.position.y = 0.24 + sin(Time.get_ticks_msec() * 0.0035) * 0.07
...
func _build_pickup() -> void:
	...
	if not DedicatedServer.active:
		...
		var model_scene := load(JETPACK_MODEL_PATH) as PackedScene
		if model_scene:
		...
		light.light_color = Color("#58d5ff")
...
func _fit_model(root: Node3D) -> void:
	...
	var fit_scale := 0.9 / maxf(longest, 0.001)
...
func _on_body_entered(body: Node3D) -> void:
	if body.get("alive") != true or body.get("mode_infected") == true or not body.has_method("acquire_jetpack"):
		return
	if body.acquire_jetpack():
